  ...
  return (
    <header className="relative z-30 flex h-16 shrink-0 items-center gap-4 border-b border-sky-400/15 bg-gradient-to-b from-[#0b1a33]/95 to-[#071224]/90 px-4 backdrop-blur-md">
      ...
      <div className="absolute left-1/2 flex -translate-x-1/2 items-center gap-6">
        <Clock />
        <div className={`flex items-center gap-2.5 rounded-full border px-4 py-1.5 ${broadcast ? 'border-rose-400/70 bg-rose-500/20 glow-red' : 'border-amber-400/60 bg-amber-500/15 glow-amber'}`}>
          <span className={`h-2.5 w-2.5 rounded-full blink ${broadcast ? 'bg-rose-400' : 'bg-amber-400'}`} />
          <span className={`font-hud text-sm font-bold uppercase tracking-[0.18em] ${broadcast ? 'text-rose-100 text-glow-red' : 'text-amber-100 text-glow-amber'}`}>
            {broadcast ? 'Cyclone Evacuation — Active' : 'Cyclone Alert — Active'}
          </span>
        </div>
      </div>

      <div className="ml-auto flex items-center gap-3">
        <button className="hidden min-[1600px]:flex items-center gap-2 rounded-lg border border-sky-400/20 bg-slate-900/40 px-3 py-1.5 text-xs text-slate-200 cursor-pointer hover:border-cyan-400/50">
